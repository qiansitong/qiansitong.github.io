(function($,doc){
	...
	$.fn.calendar=function(options,callback){
		...
		this.each(function(i, element) {
			...
			calendarBox+='<span class="title" xid="'+options["year"]+'-'+options["month"]+'">'+options["year"]+'年'+options["month"]+'月</span></div>';
			...
			var calendars = document.getElementsByClassName(element.className);
			for(var i=0;i<calendars.length;i++){
				calendars[i].innerHTML=calendarBox;
			}
